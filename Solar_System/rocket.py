...
def getColor(x, r, d, h, colors, throttle):
	y = .5*x/d +.5
	try:
		val = 255*colors[int((.5*h/r +.5)*len(colors))][int(y*len(colors[0]))]/1.5
	except: return ([], False)

	if colors == end and throttle:
		return ([255,1.5*val,1.5*val*.7], True)
	elif colors == end:
		return ([30,30,30], True)
	elif colors == side and throttle:
		#print([val,val*(y),val*(y)])
		return ([min(255,val/abs(y**.3)),val,val], True)
	elif colors == top and throttle:
		return ([min(255,val/abs(y**.3)),val,val], True)
	else:
...
def drawEnd(D, r, x, d, alpha, h, surf, throttle):
	angle = toAngle((x, h), surf.get_size()[0], surf.get_size()[1])
	offset = getOffset(D, d, angle, alpha, h)

	col, success = getColor(offset, r, r, h, end, throttle)
	if success:
...
def drawTop(D, r, x, d, alpha, h, surf, throttle):
	angle = toAngle((x, h), surf.get_size()[0], surf.get_size()[1])
	offset = getOffset(D, d*coneStart, angle, alpha, h)

	col, success = getColor(d*coneStart, r, abs(d), h, top, throttle)
	if success:
		pos = toScreen(angle, adjustHeight(D, d*coneStart, offset, alpha, h), surf.get_size()[0], surf.get_size()[1])
		pygame.draw.rect(surf, col, (pos[0], pos[1], pixel, pixel + fill))

def drawSide(D, r, screenX, d, alpha, h, surf, throttle):
	coneWeight = 1
...
def draw(alpha, surf, light, throttle):
	...
	for h in range(-r, r, pixel):
		...
		for x in range(top3[0], top4[0], pixel):
			drawTop(D, r, x, -d, alpha + pi/2, h, surf, throttle)
	...
	for h in range(-r, r, pixel):
		...
		for x in range(pos2[0], pos3[0], pixel):
			drawSide(D, r, x, d, alpha, h, surf, throttle)

		for x in range(pos4[0], pos1[0], pixel):
			drawSide(D, -r, x, d, alpha, h, surf, throttle)

		for x in range(pos1[0], pos2[0], pixel):
			drawEnd(D, r, x, d, alpha - pi/2, h, surf, throttle)
...
if __name__ == "__main__":
	...
	while not done:
		...
		draw(alpha, screen, 0, keys[pygame.K_w])
